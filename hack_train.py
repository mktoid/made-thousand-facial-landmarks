...
def main(args):
    ...
    model = torch.hub.load('facebookresearch/WSL-Images', 'resnext101_32x8d_wsl') # models.resnext50_32x4d(pretrained=True) # resnet18(pretrained=True)
    ...
    for epoch in range(args.epochs):
        ...
        print("Epoch #{:2}:\ttrain loss: {:5.4}\tval loss: {:5.4}\tval mse: {:5.4}".format(epoch, train_loss, val_loss, val_loss_mse))
        ...
        if 1 == 1: #val_loss < best_val_loss: #save results of all epoch to check several at kaggle
            ...
            create_submission(args.data, test_predictions, f"{args.name}_submit_"+ str(epoch) +".csv")
